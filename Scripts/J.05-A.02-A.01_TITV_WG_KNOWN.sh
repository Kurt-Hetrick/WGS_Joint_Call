...
>| $CORE_PATH/$OUT_PROJECT/REPORTS/TI_TV/WHOLE_GENOME/$SM_TAG"_WG_Known_titv.txt"

END_TITV_WG_KNOWN=`date '+%s'`

HOSTNAME=`hostname`

echo $SM_TAG"_"$IN_PROJECT",L.01,TITV_WG_KNOWN,"$HOSTNAME","$START_TITV_WG_KNOWN","$END_TITV_WG_KNOWN \
>> $CORE_PATH/$IN_PROJECT/REPORTS/$IN_PROJECT".WALL.CLOCK.TIMES.csv"

# echo zcat $$CORE_PATH/$OUT_PROJECT/TEMP/$SM_TAG".QC.WG.Known.TiTv.vcf.gz" \
# | $SAMTOOLS_DIR/bcftools/vcfutils.pl qstats /dev/stdin \
# \>\| $CORE_PATH/$OUT_PROJECT/REPORTS/TI_TV/WHOLE_GENOME/$SM_TAG"_WG_Known_titv.txt" \
